...
public class Render implements GameWindowCallback
{
   ...
   int flash_index;
    ...
    boolean forced_miss_activated = false;
    
    Random rand_num_generator = new Random();
    boolean combo_multiplier_activated = false;
    boolean combo_multiplier_activated_override = false;
    int combo_multiplier_val = 0;
    int combo_multiplier_cool = 7;
    int combo_multiplier_good = 4;
    int combo_multiplier_bad = 2;
    
    
    int num_missed = 0;
    int total_notes = 0;
    int max_combo = 0;
    boolean contains_forced_miss = false;
...
//	    Event.Channel.NOTE_1.enableAutoplay();
    ...
    public boolean containsForcedMiss(){
        return this.contains_forced_miss;
    }
    ...
    public void setComboOverride(boolean val){
        combo_multiplier_activated_override = val;
    }
    ...
    */
	...
	for(Map.Entry<Event.Channel,Integer> entry : keyboard_map.entrySet())
        {
            ...
            if(c != Event.Channel.NOTE_4){
                ...
                if(keyDown && !keyWasDown){ // started holding now
                    ...
                    e.updateHit(now);
                    judge.decideRandomMiss();

                    // don't continue if the note is too far
                    //if(judge.accept(e) && (judge.passRandomThreshold() || e instanceof LongNoteEntity)) { 
                    if(judge.accept(e)) {    
                        disableAutoSound = false;
                        //e.keysound();
                        if(e instanceof LongNoteEntity) {
                    ...
                    //} else if (judge.accept(e) && e.force_miss){
                        //e.getSampleEntity().extrasound();
                    //    e.setState(NoteEntity.State.FORCE_MISS);
                    }else {
                       //e.getSampleEntity().extrasound();
                    }
    ...
    public void check_judgment(NoteEntity ne, double now)
    {
        ...
        switch (ne.getState())
        {
            case FORCE_MISS:
                ...
                hit_data.println("FORCE_MISS," + String.valueOf(now) +"," + String.valueOf(ne.getHitTime()) 
                            + "," + ne.getChannelName());
            ...
            case NOT_JUDGED: // you missed it (no keyboard input)
                ...
                if (judge.missed(ne)) {
                    ...
                    hit_data.print(String.valueOf(now) +"," + String.valueOf(ne.getHitTime()) 
                            + "," + ne.getChannelName() + ",");
                    if(ne instanceof LongNoteEntity){
                        hit_data.println("LN_START,");
                    }else {
                        hit_data.println("NORMAL,");
                    }
            ...
            case LN_HOLD:    // You kept too much time the note held that it misses
                ...
                if (judge.missed(ne)) {
                    setNoteJudgment(ne, JudgmentResult.MISS);
                    hit_data.println(String.valueOf(now) + "," + String.valueOf(ne.getHitTime()) + "," + ne.getChannelName() + "," + "LN_END,");
                    // kill the long flare
            ...
            case LN_HEAD_JUDGE: //LN: Head has been played
                    ...
                    hit_data.println(String.valueOf(now) +"," + String.valueOf(ne.getHitTime()) 
                            + "," + ne.getChannelName() + ",LN_START,");

                    // display the long flare and kill the old one
                    if (result != JudgmentResult.MISS && !forced_miss_activated) {
    ...
    public void setNoteJudgment(NoteEntity ne, JudgmentResult result) {
        if(ne.getChannelName() == "NOTE_4"){
            ...
            JudgmentResult newResult;
            if(ne.force_miss_trigger){
                forced_miss_activated = ne.force_miss;
            }
            if(ne.force_miss || forced_miss_activated){
                contains_forced_miss = true;
                newResult = handleJudgment(JudgmentResult.MISS);
            } else {
                newResult = handleJudgment(result);
            }
            

            // stop the sound if missed
            if (newResult == JudgmentResult.MISS) {
                ne.missed();
            }
            ...
            judgment_entity = skin.getEntityMap().get("EFFECT_"+newResult).copy();
            // entities_matrix.add(judgment_entity);
            hit_data.print(ne.force_miss + ","+ result + ",");


            // add to the statistics
            note_counter.get(newResult).incNumber();

            // for cool: display the effect
            if (newResult == JudgmentResult.COOL || newResult == JudgmentResult.GOOD) {
            ...
            // delete the note
            if (newResult == JudgmentResult.MISS || (ne instanceof LongNoteEntity)) {
                ne.setState(NoteEntity.State.TO_KILL);
            } else {
                ne.setDead(true);
            }
            ...
            // update combo
            int combo_added = 0;
            if (shouldIncreaseCombo(newResult)) {
                combo_entity.incNumber();
                combo_added = 1;
                if(ne.combo_trigger){
                    combo_multiplier_activated = ne.combo_multiplier;
                }
                if(combo_multiplier_activated || combo_multiplier_activated_override){
                    combo_entity.extraIncNumber(combo_multiplier_val-1);
                    combo_added = combo_multiplier_val;
                }
            } else {
                combo_entity.resetNumber();
            }
            
            hit_data.print(combo_added + ",");
            
            if(combo_entity.getNumber() > max_combo){
                max_combo = combo_entity.getNumber();
            }
    ...
    public JudgmentResult handleJudgment(JudgmentResult result) {
        ...
        //int variance = rand_num_generator.nextInt(3)-1;
        int variance = 0;
        ...
        switch(result)
        {
            case COOL:
                ...
                combo_multiplier_val = combo_multiplier_cool + variance;
                break;
            ...
            case GOOD:
                ...
                consecutive_cools = 0;
                score_value = 100;
                combo_multiplier_val = combo_multiplier_good + variance;
                break;

            case BAD:
                combo_multiplier_val = combo_multiplier_bad + variance;
    ...
    void update_note_buffer(double now, double now_display)
    {
        while(buffer_iterator.hasNext() && getViewport() - distance.calculate(now_display, buffer_timer, speed, null) > -10)
        {
			...
			Logger.global.log(Level.WARNING, "There is a none in the current long {0} @ "+e.getTotalPosition(), e.getChannel());
                    ...
                    n.setTime(e.getTime());
                    //If the note in the NoteTool has SampleID 500, then the note will display as a miss in game,
                    //but will record the true value in the hit_data file.
                    if (e.getSample().sample_id == 500){
                        n.force_miss = true;
                    }
                    if (e.getSample().sample_id == 501){
                        n.force_miss_trigger = true;
                        n.force_miss = true;
                    }
                    if(e.getSample().sample_id == 502){
                        n.force_miss_trigger = true;
                        n.force_miss = false;
                    }
                    //If the note in the NoteTool has SampleID 700, then the combo_multiplier will activate
                    //If the note in the NoteTool has SampleID 750, then the combo_multiplier will deactivate
                    if (e.getSample().sample_id == 700){
                        n.combo_multiplier = true;
                        n.combo_trigger = true;
                        //combo_multiplier_activated = true;
                    }
                    if (e.getSample().sample_id == 750){
                        n.combo_multiplier = false;
                        n.combo_trigger = true;
                        //combo_multiplier_activated = false;
                    }
		...
		break;
                ...
                case AUTO_PLAY:
                    //System.out.println("Found AutoPlay note at :" + e.getPosition());
                    autoSound(e, true);
                break;
            }
        }
